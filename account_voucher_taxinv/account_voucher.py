...
class account_voucher(osv.osv):
    ...
    _columns = {
        ...
        'taxinv_period_id': fields.many2one('account.period', 'Tax Invoice Period'),
    }
    ...
    def to_publish(self, cr, uid, ids, context=None):
        ...
        return self.write(cr, uid, ids, {'is_taxinv_publish': True})
    ...
    def button_reset_taxinv(self, cr, uid, ids, context=None):
        if context is None:
            context = {}
        ctx = context.copy()
        avt_obj = self.pool.get('account.voucher.taxinv')
        for id in ids:
            ...
            if voucher.type == 'payment':
                ...
                for tax in avt_obj.compute(cr, uid, id, context=ctx).values():
                    tax.update({'partner_id': voucher.partner_id and voucher.partner_id.id or False,
                               'date': voucher.date})
